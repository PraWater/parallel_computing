...
__global__ void scoresCount(float *d_final_scores, int *d_count, int N) {
  int myID = blockIdx.x * blockDim.x + threadIdx.x;
  if (myID < N) {
    if (d_final_scores[myID] > 0)
      atomicAdd(&d_count[0], 1);
    else if (d_final_scores[myID] < 0)
      atomicAdd(&d_count[1], 1);
    else
      atomicAdd(&d_count[2], 1);
  }
}
...
int main() {
  ...
  int no_of_reviews{0};
  // Reading the file line by line while appending the scores of each word in
  // the review and mapping the ASINs to integers.
  ...
  CUDA_CHECK_RETURN(cudaMalloc(&d_count, 3 * sizeof(int)));
  ...
  CUDA_CHECK_RETURN(cudaMemset(d_count, 0, 3 * sizeof(int)))
  ...
  // Counts the scores to find the number in each type.
  grid = ceil(no_of_reviews * 1.0 / BLOCK_SIZE);
  scoresCount<<<grid, BLOCK_SIZE>>>(d_final_scores, d_count, no_of_reviews);

  CUDA_CHECK_RETURN(cudaEventRecord(stop));
  CUDA_CHECK_RETURN(cudaEventSynchronize(stop));
  milliseconds = 0;
  CUDA_CHECK_RETURN(cudaEventElapsedTime(&milliseconds, start, stop));
  printf("Timetaken for scoresCount: %f\n", milliseconds);
  CUDA_CHECK_RETURN(cudaEventDestroy(start));
  CUDA_CHECK_RETURN(cudaEventDestroy(stop));

  // Host memory
  int *h_count = (int *)malloc(3 * sizeof(int));
  CUDA_CHECK_RETURN(
      cudaMemcpy(h_count, d_count, 3 * sizeof(int), cudaMemcpyDeviceToHost));

  std::cout << "Total no. of reviews: " << no_of_reviews << std::endl
            << "Positive: " << h_count[0] << " Negative: " << h_count[1]
            << " Neutral: " << h_count[2] << std::endl;

  // Free device memory
  CUDA_CHECK_RETURN(cudaFree(d_scores));
  CUDA_CHECK_RETURN(cudaFree(d_indices));
  CUDA_CHECK_RETURN(cudaFree(d_final_scores));
  CUDA_CHECK_RETURN(cudaFree(d_count));
  free(h_count);
}
